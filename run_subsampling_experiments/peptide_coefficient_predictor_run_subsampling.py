####################################
# Script for training Pepper model by randomly subsampling training runs
# Author: Ayse Dincer
...
parser.add_argument('--subsample_size', help='Number of train runs to subsample', required = True)
...
#Subsample from the train runs
print("Subsampling ", subsample_size, " train runs...")
train_runs = np.random.choice(train_runs, int(subsample_size / 2))
...
all_results_df.T.to_csv("trained_models/" + args.output_file + '_result_scores_random_subsampling_' + str(subsample_size) + "random_run" + str(random_run) + ".tsv", sep = '\t')
