####################################
# Script for training a linear coefficient predictor using the same loss function as the neural network but using the number of occurences of each letter in the alphabet
...
formatted_peptide_sequences = np.zeros((data_df.shape[0], 20))
...
for p in range(data_df['Peptide'].shape[0]):
    ...
    for i in range(len(peptide)):
        one_mer = peptide[i]
        encoded_vector[all_kmers.index(one_mer)] += 1
...
def define_model():
    ...
    #Define network
    inputs =  Input(shape=(20), name = 'sequence')
...
all_results_df.to_csv('results/NCI-60_1mer_count_regression_random_run' + str(args.random_run) + '.tsv', sep = '\t')
