###################################
# Script for training a linear coefficient predictor using the same loss function as the neural network but using the number of occurences of each 3-mer
...
#Record all 3-mer sequences
for a1 in alphabet:
    for a2 in alphabet:
        for a3 in alphabet:
            kmer = str(a1) + str(a2) + str(a3)
            all_kmers.append(kmer)


#Record occurences of each k-mer
formatted_peptide_sequences = np.zeros((data_df.shape[0], 8000))
...
for p in range(data_df['Peptide'].shape[0]):
    ...
    for i in range(len(peptide) - 2):    
        three_mer = peptide[i:i+3]
        encoded_vector[all_kmers.index(three_mer)] += 1
...
def define_model():
    ...
    #Define network
    inputs =  Input(shape=(8000), name = 'sequence')
...
all_results_df.to_csv('results/NCI-60_3mer_count_regression_random_run' + str(args.random_run) + '.tsv', sep = '\t')
