...
#SPlit runs into train/test sets 
...
default_test_peptide_loss_new = K.eval(peptide_loss(K.constant(joined_intensities_val),
...
predicted_test_coeffs = model.predict([X_val, C_val,  
                                 np.ones((X_val.shape[0], joined_intensities.shape[1]))]).ravel()
...
#Define minimization problem
alpha_final_test = alpha_initial_val.astype(np.float32)
...
final_test_peptide_loss_new = K.eval(peptide_loss(K.constant(joined_intensities_val),
...
all_results_df.T.to_csv('tuning_results/NCI-60_label_noise_simulation_results_robust_model_noise_ratio_' + args.label_noise_ratio +'lam_' + str(lam) + '_run' + str(random_run) + '.tsv', sep = '\t')
