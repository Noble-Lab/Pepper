####################################
# Script for training a robust neural network model for inferring peptide coefficients from peptide sequences
# Injected artificial label noise
# Author: Ayse Dincer
...
parser.add_argument('--filter_size', required = True)
parser.add_argument('--n_filters', required = True)
parser.add_argument('--n_layers', required = True)
parser.add_argument('--n_nodes', required = True)
...
parser.add_argument('--label_noise_ratio', help='Ratio of labels to shuffle', required = True)
...
############################
## Random subsampling experiment

#Record true P_train
P_train_true = P_train
P_val_true = P_val

if float(args.label_noise_ratio) == 1.0:

    P_train_old = P_train.copy()
    np.random.shuffle(P_train)

    print("Shuffled count: ",np.sum([1 for i in range(len(P_train)) if np.array(P_train[i]) != np.array(P_train_old[i])]))

    #Also shuffle P_val
    P_val_old = P_val.copy()
    np.random.shuffle(P_val)

elif float(args.label_noise_ratio) != 0.0:

    #Shuffle labels
    nonshuffle_proteins, shuffle_proteins = train_test_split((np.arange(len(np.unique(P_train_true)))), test_size= float(args.label_noise_ratio))


    #Define train/validation/test peptide pairs
    shuffle_indices = np.concatenate([list(np.where(P_train_true == (np.arange(len(np.unique(P_train_true))))[p])[0]) for p in shuffle_proteins])

    P_train = P_train_true
    P_train_old = P_train
    shuffled_P = np.array(P_train)[shuffle_indices]
    np.random.shuffle(shuffled_P)
    P_train = np.array(P_train)
    P_train[shuffle_indices] = shuffled_P

    print("Shuffled ratio: ", len(shuffle_indices) / len(P_train))
    print("Shuffled count: ",np.sum([1 for i in range(len(P_train)) if np.array(P_train[i]) != np.array(P_train_old[i])]))


    #Also shuffle P_val
    nonshuffle_proteins, shuffle_proteins = train_test_split((np.arange(len(np.unique(P_val_true)))), test_size= float(args.label_noise_ratio))


    shuffle_indices = np.concatenate([list(np.where(P_val_true == (np.arange(len(np.unique(P_val_true))))[p])[0]) for p in shuffle_proteins])

    P_val = P_val_true
    P_val_old = P_val
    shuffled_P = np.array(P_val)[shuffle_indices]
    np.random.shuffle(shuffled_P)
    P_val = np.array(P_val)
    P_val[shuffle_indices] = shuffled_P

    print("Shuffled ratio val: ", len(shuffle_indices) / len(P_val))
    print("Shuffled count val: ",np.sum([1 for i in range(len(P_val)) if np.array(P_val[i]) != np.array(P_val_old[i])]))


    ####################################
    #Check how many siblings
    no_of_new_siblings = 0

    for p in np.unique(P_train):

        siblings = np.where(P_train_old == p)[0]
        new_labels = list(P_train[siblings])
        duplicates = [x for x in new_labels if new_labels.count(x) > 1]
        duplicate_counts = len(duplicates)
        no_of_new_siblings += duplicate_counts

    print("No of remaining siblings ", no_of_new_siblings)
    print("No of remaining siblings ", no_of_new_siblings / len(P_train))
...
for i in range(len(np.unique(P_train))):
    ...
    mean_abundances = np.mean(Q_train.iloc[all_sub_peptides], axis = 0)
...
for i in range(len(np.unique(P_val))):
    ...
    mean_abundances = np.mean(Q_val.iloc[all_sub_peptides], axis = 0)
...
for i in range(len(np.unique(P_test))):
    ...
    mean_abundances = np.mean(Q_test.iloc[all_sub_peptides], axis = 0)
...
class CustomLossLayer(layers.Layer):
    def __init__(self):
        ...
        #Define S variables that are trainable
        self.S = tf.Variable(np.random.rand(Q_train.shape[0], Q_train.shape[1]), 
                              trainable = True, 
                              dtype = 'float32')
    ...
    def peptide_loss(self, y_true, y_pred):
        ...
        q_input = y_true[:, :-2] #dimension (batch_size, K)

        label_input = y_true[:, -2] #dimension (batch_size, 1)
        label_input = tf.cast(label_input, tf.int32)

        peptide_index = y_true[:, -1] #dimension (batch_size, 1), vector of peptide indices
        peptide_index = tf.cast(peptide_index, tf.int32)
        ...
        #Find the corresponding s values
        s_peptides = tf.gather(K.abs(self.S), peptide_index, axis = 0)

        #Calculate the difference values
        differences = (q_input - adjusted_abundances) - s_peptides
        ...
        #Calculate the sparsity loss for s1
        s_peptides = s_peptides * zero_peptides * zero_coeffs
        sparsity_loss = K.sum(K.sqrt(K.sum(K.square(K.abs(s_peptides)), 1) + 1e-20))
        sparsity_loss = sparsity_loss / all_runs

        return avg_loss + lam * sparsity_loss
...
def peptide_loss(x, c_pred, alphas):
    ...
    q_input = x[:, :-2] #dimension (batch_size, K)
    #label_input is the protein labels for each peptide
    label_input = x[:, -2] #dimension (batch_size, 1)
    label_input = tf.cast(label_input, tf.int32)
...
def define_model():
    ...
    inputs_label = Input(shape=(len(train_runs) + 2,), name = 'y_true')
...
joined_intensities = np.column_stack((Q_train, P_train, np.arange(Q_train.shape[0]) )).astype(np.float32)
print("Train joined intensities ", joined_intensities.shape)

joined_intensities_val = np.column_stack((Q_val, P_val, np.arange(Q_val.shape[0]))).astype(np.float32)
print("Val joined intensities ", joined_intensities_val.shape)

joined_intensities_test = np.column_stack((Q_test, P_test, np.arange(Q_test.shape[0]) )).astype(np.float32)
...
S_matrix = K.eval(model.layers[-1].get_vars_S())
print("Final S matrix ", S_matrix)
print("Final S sparsity ", np.count_nonzero(~np.isnan(S_matrix[np.abs(S_matrix) > 0.05])) / (S_matrix.shape[0] * S_matrix.shape[1]))
...
#Record results
all_result = [final_train_peptide_loss_new, final_train_improvement, final_test_peptide_loss_new, final_test_improvement]

all_results_df = pd.DataFrame(all_results, index = ['Train loss', 'Train improvement', 'Test loss', 'Test improvement'])
all_results_df.T.to_csv('results/NCI-60_label_noise_simulation_results_robust_model_noise_ratio_' + args.label_noise_ratio +'_run' + str(random_run) + '.tsv', sep = '\t')
