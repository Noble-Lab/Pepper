####################################
# Script for loading pretrained Pepper model and transferring to external datasets
# Author: Ayse Dincer
...
X = (X / X.sum(axis=0, keepdims=1)) * X.shape[0] / 10
...
train_runs, test_runs = train_test_split((np.arange(q_df.shape[1])), 
                                   test_size=0.2, random_state=12345)
...
#Define model
def define_model(protein_count, run_count):
    ...
    class CustomLossLayer(layers.Layer):
        ...
        def __init__(self):
            ...
            if run_count == 612:
                ...
                self.alphas = tf.Variable(np.random.rand(protein_count, run_count), 
                                          trainable = True, 
                                          dtype = 'float32')
    ...
    inputs_label = Input(shape=(run_count + 1,), name = 'y_true')
...
for l in range(len(target_model.layers) - 1):
    w_target = target_model.layers[l].get_weights()
    if len(w_target) > 0:
        model.layers[l].set_weights(w_target)
...
predicted_coefficients.to_csv("trained_models/" + args.output_file + "_transfer_inferred_coefficients_run" + str(random_run) + ".tsv", sep = '\t')
...
with tf.compat.v1.Session() as session:
    ...
    step = 0
    #Train until convergence or max no of steps
    while score_diff > 0.01 and step < 1000:
